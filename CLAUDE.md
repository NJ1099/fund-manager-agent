...
아래 "장부 불변식"과 "백테스트 규율" 두 절은 **코드를 고치기 전에** 읽어야 합니다.
과거에 실제로 물렸던 버그들이 왜 그렇게 설계됐는지 설명합니다.
...
`.env`는 `.gitignore`에 등록돼 있다. 이 프로젝트에서 쓰는 키는 `ANTHROPIC_API_KEY`
하나뿐이며, 그마저도 PM 논평(선택 기능)에만 쓰인다 — 없어도 전 기능이 동작한다.
